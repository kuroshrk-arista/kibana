...
import bmfLightSvg from './assets/bmf-logo-light.svg';
...
export class DashboardPlugin
  ...
  public setup(
    ...
    if (share) {
      this.dashboardUrlGenerator = share.urlGenerators.registerUrlGenerator(
        createDashboardUrlGenerator(async () => {
          ...
          return {
            appBasePath: coreStart.application.getUrlForApp('bigswitch'),
    ...
    const {
      ...
      baseUrl: core.http.basePath.prepend('/app/bigswitch'),
      defaultSubUrl: `#${DashboardConstants.LANDING_PAGE_PATH}/Flows`,
      storageKey: `lastUrl:${core.http.basePath.get()}:bigswitch`,
    ...
    const app: App = {
      ...
      order: 2500,
      icon: bmfLightSvg,
      defaultPath: `#${DashboardConstants.LANDING_PAGE_PATH}/Flows`,
    ...
    urlForwarding.forwardApp(
      ...
      DashboardConstants.DASHBOARDS_ID,
      (path) => {
        const [, id, tail] = /bigswitch\/?(.*?)($|\?.*)/.exec(path) || [];
        if (!id && !tail) {
    ...
    if (home) {
      home.featureCatalogue.register({
        ...
        icon: bmfLightSvg,
        path: `/app/bigswitch#/${DashboardConstants.LANDING_PAGE_PATH}/Flows`,
        showOnHomePage: true,
